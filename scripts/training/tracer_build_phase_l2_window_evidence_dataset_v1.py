...
def build_candidate_windows(
    ...
    def exact_empirical_output(row, tol=1e-12):
        """
        Return true only when J7's actual output is numerically
        identical to the current empirical reference.

        This does not establish that the intended candidate is a
        no-op. Candidate intent is checked later from authoritative
        J4 parameters.
        """
        field_pairs = [
            ("emp_seq", "out_seq"),
            ("emp_vx", "out_vx"),
            ("emp_yaw_rate", "out_yaw_rate"),
            ("emp_body_h", "out_body_h"),
            ("emp_clearance", "out_clearance"),
            ("emp_enable", "out_enable"),
        ]

        for empirical_field, output_field in field_pairs:
            try:
                empirical = float(
                    row.get(empirical_field, "")
                )
                output = float(
            ...
            if (
                not math.isfinite(empirical)
                or not math.isfinite(output)
            ):
                return False

            if abs(output - empirical) > tol:
                return False

        return True

    # Determine no-op intent from authoritative J4 parameters.
    # The key includes context and action_id so future rollouts may
    # contain different actions in different terrain segments.
    j4_by_key = {}
    ...
    for key, rows in j4_by_key.items():
        vx_scale = mean_field(
            rows, "vx_scale"
        )
        vx_delta = mean_field(
        ...
        yaw_gain_scale = mean_field(
            rows, "yaw_gain_scale"
        )
        body_h_delta = mean_field(
            rows, "body_h_delta"
        )
        clearance_delta = mean_field(
            rows, "clearance_delta"
        )
        ...
        is_noop_key = (
            available
            and abs(vx_scale - 1.0) < 1e-6
            and abs(vx_delta) < 1e-6
            and abs(yaw_gain_scale - 1.0) < 1e-6
            and abs(body_h_delta) < 1e-6
            and abs(clearance_delta) < 1e-6
        )
    ...
    for row in j7_rows:
        if (
            not known_context(row)
            or timestamp(row) is None
        ):
            continue
        ...
        if (
            is_true(row.get("j7_accept"))
            and source == "projected"
        ):
            tagged = dict(row)
            tagged["_l2_route_kind"] = (
                "accepted_projected"
            )
            eligible.append(tagged)
            continue

        # This route is only provisionally eligible here.
        # It is retained later only when authoritative J4
        # parameters prove that the intended action was no-op.
        fallback_key = (
        ...
        if (
            source == "empirical"
            and exact_empirical_output(row)
            and fallback_key in noop_keys
        ):
            tagged = dict(row)
            tagged["_l2_route_kind"] = (
                "exact_empirical_fallback"
            )
            eligible.append(tagged)
    ...
    for group_idx, group in enumerate(groups, start=1):
        ...
        if route_kinds == {
            ...
            route_kind = "accepted_projected"
        else:
            route_kind = (
                "mixed_noop_equivalent"
            )
        ...
        # An empirical fallback is valid as a control window only
        # when J4 proves that the intended candidate itself was an
        # exact no-op. Empirical fallback from a real intervention
        # must never be labeled as candidate exposure.
        if (
            route_kind != "accepted_projected"
            and not is_noop
        ):
        ...
        if (
            ...
            window_type = (
                "empirical_fallback_noop_window"
            )
        elif (
            route_kind
            == "mixed_noop_equivalent"
            ...
            window_type = (
                "mixed_route_noop_window"
            )
        else:
            window_type = (
                "accepted_candidate_exposure"
            )
...
def main():
    ...
    lines = [
        "# TRACER Phase-L2 Window Action Evidence Summary v1",
        "",
        f"- source K6: `{k6_path}`",
    ...
    lines += [
        ...
        "- Intervention candidate rows require realized J7 projected exposure.",
        "- Exact empirical fallback may be retained only as a J4-proven no-op control when J7 output is numerically identical to the empirical reference.",
