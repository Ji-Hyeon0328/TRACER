...
def compute_group_targets(rows, std_penalty, risk_penalty, reach_bonus):
    ...
    for key, g in by_key.items():
        rs = g["rows"]
        scores = [ff(r.get("preference_score_seed")) for r in rs]
        risks = [ff((r.get("ram_labels") or {}).get("future_risk_proxy")) for r in rs]
        reaches = [1.0 if (r.get("reward_components") or {}).get("reached_stop_distance") else 0.0 for r in rs]
        finals = [ff((r.get("reward_components") or {}).get("final_rel_dist")) for r in rs]
        holds = [ff((r.get("reward_components") or {}).get("hold_reward")) for r in rs]
...
def build_examples(rows, group_targets, obj_ckpt, obj_model, ram_ckpt, ram_model):
    ...
    with torch.no_grad():
        for r in rows:
            ...
            # Clean deployable input:
            # causal features + Objective prediction + RAM prediction.
            # Group statistics are used only for target labels, not as input.
...
def main():
    ...
    ap.add_argument("--rollouts", default="data/phase_b_training_pipeline_v3_merged/phase_b_rollout_index_v1.jsonl")
    ...
    ap.add_argument("--out-dir", default="artifacts/phase_b_beta_ram_candidate_policy_v3_robust_clean")
    ap.add_argument("--report-json", default="reports/phase_b_beta_ram_candidate_policy_eval_v3_robust_clean.json")
    ap.add_argument("--report-md", default="reports/phase_b_beta_ram_candidate_policy_eval_v3_robust_clean.md")
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for _, wrs in by_world.items():
            if len(wrs) < 2:
                continue
            for _ in range(min(64, len(wrs) * 2)):
    ...
    ckpt = {
        "schema": "phase_b_beta_ram_candidate_policy_v3_robust_clean",
        ...
        "group_targets_for_training_only": group_targets,
        "action_vocab": action_vocab,
        "world_vocab": world_vocab,
        "best_epoch": best["epoch"],
        "best_score": best["score"],
        "note": "Robust-clean candidate scorer. Group statistics are target labels only, not runtime input features.",
    }
    torch.save(ckpt, out_dir / "model.pt")

    report = {
        "schema": "phase_b_beta_ram_candidate_policy_train_report_v3_robust_clean",
    ...
    lines = []
    lines.append("# Phase-B β/RAM Candidate Policy v3 Robust-Clean")
    lines.append("")
    ...
    lines.append("")
    lines.append("Group statistics are used only as training labels, not as candidate policy inputs.")
    lines.append("")
