...
class ResidualActor(nn.Module):
    def __init__(self, input_dim, output_dim=6, hidden=96):
        super().__init__()
...
def feature_vec(r):
    ...
    # deployable side-info: v8 scorer can provide this at runtime.
...
def main():
    ...
    ap.add_argument("--dataset", default="data/phase_b_theta_residual_actor_v0/phase_b_theta_residual_actor_seed_v0.jsonl")
    ap.add_argument("--out-dir", default="artifacts/phase_b_theta_residual_actor_v0")
    ap.add_argument("--report-json", default="reports/phase_b_theta_residual_actor_v0.json")
    ap.add_argument("--report-md", default="reports/phase_b_theta_residual_actor_v0.md")
    ...
    model = ResidualActor(input_dim=X_train.shape[1])
    ...
    with torch.no_grad():
        for r in rows:
            ...
            p = model(x)[0].tolist()
            y = target_vec(r)
            key = f"{r['world']}::{r['anchor_action']} -> {r['target_action']}"
            group[key].append((p, y, r))
    ...
    for key, vals in sorted(group.items()):
        ...
        group_summary.append({
            ...
            "n": len(vals),
            "mse_mean": mse,
            "pred_delta_norm": {k: pred_mean[i] for i, k in enumerate(THETA_KEYS)},
            "target_delta_norm": {k: targ_mean[i] for i, k in enumerate(THETA_KEYS)},
        })
    ...
    ckpt = {
        "schema": "phase_b_theta_residual_actor_v0",
        ...
        "feature_schema": [
            "world_onehot[earth,stairs_single,tracer_sponge_firm_flat]",
            "beta_v,beta_s,beta_e",
            "anchor_theta_scaled[vx_far,vx_near,goal_slow_distance,goal_stop_distance,body_height,swing_clearance]",
    ...
    report = {
        "schema": "phase_b_theta_residual_actor_train_report_v0",
    ...
    lines = []
    lines.append("# Phase-B θ Residual Actor v0")
    lines.append("")
