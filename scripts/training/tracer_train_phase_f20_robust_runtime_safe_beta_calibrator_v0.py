...
OUT_MODEL = Path("models/phase_f/f20_robust_runtime_safe_beta_calibrator_ridge_v0.json")
OUT_MD = Path("reports/phase_f/f20_robust_runtime_safe_beta_calibrator_summary_v0.md")
...
tags = sorted(set(r.get("f19_base_tag", "unknown") for r in rows))

tag_summary = []
for tag in tags:
    idx = [i for i, r in enumerate(rows) if r.get("f19_base_tag", "unknown") == tag]
...
loco = []
for hold in tags:
    train_rows = [r for r in rows if r.get("f19_base_tag", "unknown") != hold]
    test_rows = [r for r in rows if r.get("f19_base_tag", "unknown") == hold]
...
OUT_MODEL.write_text(json.dumps({
    "version": "phase_f20_robust_runtime_safe_beta_calibrator_ridge_v0",
...
with open(OUT_MD, "w") as f:
    f.write("# TRACER Phase-F20 Robust Runtime-Safe Beta Calibrator v0\n\n")
    f.write("This trains a diagnostic beta calibrator using runtime-available D7/log features and Phase-F18 robust median true-metric beta targets.\n\n")
    ...
    f.write("\n## Safe interpretation\n\n")
    f.write("This is the robust-target counterpart of F14/F10. It reduces mean-target sensitivity to rollout outliers, but it is still diagnostic and should not be deployed without broader terrain/objective diversity.\n")
