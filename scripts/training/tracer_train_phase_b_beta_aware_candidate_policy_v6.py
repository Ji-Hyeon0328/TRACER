    ...
    for r in rows:
        ...
        for k in feat.keys():
            if k.startswith("metric:") or k.startswith("reward:"):
                continue
            # 이번 v6는 theta feature를 포함한다.
            # candidate action별 차이를 학습해야 하기 때문.
            names.add(k)
...
def main():
    ...
    ap.add_argument("--out-dir", default="artifacts/phase_b_beta_aware_candidate_policy_v6")
    ap.add_argument("--report-json", default="reports/phase_b_beta_aware_candidate_policy_v6.json")
    ap.add_argument("--report-md", default="reports/phase_b_beta_aware_candidate_policy_v6.md")
    ...
    extra_names = [
        ...
        "beta_s",
        "beta_e",
        "group_reach_rate_for_training",
        "group_mean_final_dist_for_training",
        "group_mean_drift_for_training",
    ]
    ...
    ckpt = {
        "schema": "phase_b_beta_aware_candidate_policy_v6",
        ...
        "target_meta_for_training_only": target_meta,
        "note": "Beta-aware candidate policy trained on task-aware beta reward target. Extra group fields are training-time diagnostics and should be replaced by runtime beta/context values in the next actor version.",
    }
    torch.save(ckpt, out_dir / "model.pt")

    report = {
        "schema": "phase_b_beta_aware_candidate_policy_v6_report",
    ...
    lines = []
    lines.append("# Phase-B β-aware Candidate Policy v6")
    lines.append("")
    ...
    lines.append("")
    lines.append("| world::action | n | pred | target_task_beta | target_proxy | reach | final | drift |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|")
