...
def main():
    ...
    for r in rows:
        ...
        progress_score = clamp01(progress / 0.50)

        # v1c: final and drift are now much more important.
        final_score = clamp01(1.0 - final_dist / 1.50)
        drift_score = clamp01(1.0 - drift_proxy / 1.25)

        # Deploy-valid score:
        # positive only if it reaches AND stays reasonably close.
        deploy_valid_score = (
            2.0 * reach_score
            + 0.5 * progress_score
            + 1.5 * final_score
            ...
            - 0.75 * float(warning)
            - 0.8 * max(0.0, final_dist - 1.0)
            - 0.8 * max(0.0, drift_proxy - 0.75)
        )
        ...
        deploy_valid_success = bool(
            reached
            and not hard_invalid
            and final_dist < 1.0
            and drift_proxy < 0.75
        )

        out_rows.append({
            "schema": "phase_b_validity_episode_label_v1c",
            "world": r.get("world"),
            "action": r.get("action"),
            ...
            "final_score_v1c": final_score,
            "drift_score_v1c": drift_score,
            "deploy_valid_score_v1c": deploy_valid_score,
            "valid_success_v1b_compatible": valid_success,
            "deploy_valid_success_v1c": deploy_valid_success,
        })
    ...
    report = {
        "schema": "phase_b_validity_episode_label_report_v1c",
        "input": args.calibrated_invalid_jsonl,
        "num_episodes": len(out_rows),
        "mean_deploy_valid_score_v1c": sum(r["deploy_valid_score_v1c"] for r in out_rows) / max(1, len(out_rows)),
        "deploy_valid_success_rate_v1c": sum(r["deploy_valid_success_v1c"] for r in out_rows) / max(1, len(out_rows)),
        "by_action": {},
    }
    ...
    for key, rs in sorted(by_action.items()):
        ...
        report["by_action"][key] = {
            "n": n,
            "mean_deploy_valid_score_v1c": sum(r["deploy_valid_score_v1c"] for r in rs) / max(1, n),
            "deploy_valid_success_rate_v1c": sum(r["deploy_valid_success_v1c"] for r in rs) / max(1, n),
            "valid_success_rate_v1b_compatible": sum(r["valid_success_v1b_compatible"] for r in rs) / max(1, n),
    ...
    lines = []
    lines.append("# Phase-B Validity Episode Labels v1c")
    lines.append("")
    lines.append(f"- Episodes: `{report['num_episodes']}`")
    lines.append(f"- Mean deploy-valid score: `{report['mean_deploy_valid_score_v1c']:.3f}`")
    lines.append(f"- Deploy-valid success rate: `{report['deploy_valid_success_rate_v1c']:.3f}`")
    lines.append("")
    lines.append("| world::action | n | deploy_success | score_v1c | reach | hard_invalid | warning | final | drift_proxy |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|")
    for key, v in sorted(
        report["by_action"].items(),
        key=lambda kv: kv[1]["mean_deploy_valid_score_v1c"],
        reverse=True,
    ):
        lines.append(
            f"| {key} | {v['n']} | {v['deploy_valid_success_rate_v1c']:.3f} | "
            f"{v['mean_deploy_valid_score_v1c']:.3f} | {v['reach_rate']:.3f} | "
