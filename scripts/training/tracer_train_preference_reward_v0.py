...
def build_dataset(rows: list[dict[str, Any]], min_confidence: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    xw, xl, weights = [], [], []
...
def main() -> None:
    ...
    ap.add_argument(
        "--pairs-jsonl",
        default="data/preference_datasets/tracer_rollout_preference_pairs_with_manifest_v0_filtered.jsonl",
    )
    ap.add_argument("--out-dir", default="artifacts/preference_reward_v0")
    ...
    xw, xl, weights = build_dataset(rows, args.min_confidence)
    ...
    for epoch in range(args.epochs):
        ...
        # Bradley-Terry: winner should score higher than loser.
    ...
    for i, r in enumerate(rows):
        ...
        preview.append({
            ...
            "winner_score": float(sw[len(preview)]),
            "loser_score": float(sl[len(preview)]),
            "score_margin": float(sw[len(preview)] - sl[len(preview)]),
        })
