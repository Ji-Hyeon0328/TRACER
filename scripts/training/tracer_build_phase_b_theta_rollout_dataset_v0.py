...
def compact_action(pred, summary):
    profile = pred.get("profile", {})
    return {
        "theta_profile_name": profile.get("name", summary.get("theta_profile_name", "")),
        "theta_guard_reasons": pred.get("guard_reasons", summary.get("theta_guard_reasons", [])),
        ...
        "projected_prediction": pred.get("projected_prediction", {}),
    }
...
def main():
    ...
    ap.add_argument("--adaptive-theta-root", required=True)
    ap.add_argument("--out-jsonl", required=True)
    ap.add_argument("--out-summary-json", required=True)
    args = ap.parse_args()

    root = Path(args.adaptive_theta_root)
    adaptive_csv = root / "adaptive_theta_summary.csv"

    if not adaptive_csv.is_file():
        raise FileNotFoundError(f"adaptive_theta_summary.csv not found: {adaptive_csv}")
    ...
    for row in rows:
        ...
        pred_path = ep_dir / "theta_prediction.json"
        ...
        if not pred_path.is_file():
            print(f"[WARN] missing theta prediction json: {pred_path}")
            pred = {}
        else:
        ...
        action = compact_action(pred, summary)
        ...
        ex = {
            "episode_index": ep,
            "adaptive_theta_root": str(root),
            "paths": {
                "risk_json": str(risk_path),
                "theta_prediction_json": str(pred_path),
            ...
            "dataset_type": "phase_b_adaptive_theta_rollout_v0",
        }
    ...
    for ex in examples:
        guards = ex["action"].get("theta_guard_reasons", [])
        if isinstance(guards, list):
            key = "; ".join(str(x) for x in guards)
        else:
            key = str(guards)
        summary_out["theta_guard_counts"].setdefault(key, 0)
        summary_out["theta_guard_counts"][key] += 1
        summary_out["vx_far_values"].append(ex["action"]["vx_far"])
        summary_out["max_yaw_values"].append(ex["outcome"]["max_abs_mpc_yaw_rate"])
