...
OUT_MODEL = Path("models/phase_f/f14_grouped_runtime_safe_beta_calibrator_ridge_v0.json")
OUT_MD = Path("reports/phase_f/f14_grouped_runtime_safe_beta_calibrator_summary_v0.md")
...
tags = sorted(set(r.get("f13_base_tag", "unknown") for r in rows))

tag_summary = []
for tag in tags:
    idx = [i for i, r in enumerate(rows) if r.get("f13_base_tag", "unknown") == tag]
...
loco = []
for hold in tags:
    train_rows = [r for r in rows if r.get("f13_base_tag", "unknown") != hold]
    test_rows = [r for r in rows if r.get("f13_base_tag", "unknown") == hold]
...
OUT_MODEL.write_text(json.dumps({
    "version": "phase_f14_grouped_runtime_safe_beta_calibrator_ridge_v0",
...
with open(OUT_MD, "w") as f:
    f.write("# TRACER Phase-F14 Grouped Runtime-Safe Beta Calibrator v0\n\n")
    f.write("This trains a diagnostic beta calibrator using runtime-available D7/log features and Phase-F12 grouped true-metric beta targets.\n\n")
    ...
    f.write("\n## Safe interpretation\n\n")
    f.write("This is the grouped-target counterpart of F10. It should reduce repeat-level label noise, but it is still diagnostic and should not be deployed without broader terrain/objective diversity.\n")
