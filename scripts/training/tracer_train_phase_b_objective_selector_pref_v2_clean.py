...
def main():
    ...
    ap.add_argument("--out-dir", default="artifacts/phase_b_objective_selector_pref_v2_clean")
    ...
    ckpt = {
        "schema": "phase_b_objective_selector_pref_v2_clean",
        ...
        "note": "Clean bootstrap preference Objective Selector. Inputs exclude post-outcome metric/reward leakage.",
    }
    torch.save(ckpt, out_dir / "model.pt")

    report = {
        "schema": "phase_b_objective_selector_pref_train_report_v2_clean",
        "pairs": args.pairs,
        "out_dir": str(out_dir),
        "num_pairs": len(rows),
        "input_dim": len(feature_names),
        ...
        "warning": "Validation is still not a deployment guarantee because pairs are derived from a small 60-rollout seed dataset. Use this as a seed model, not final evidence.",
    }
    save_json(out_dir / "train_report.json", report)

    lines = []
    lines.append("# Phase-B Objective Selector Preference Training v2 Clean")
    lines.append("")
    ...
    lines.append("")
    lines.append("Inputs are restricted to world/terrain/theta features. Post-outcome `metric:*` and `reward:*` features are excluded.")
    lines.append("")
    lines.append("This is a clean seed Objective Selector utility model, not the final converged selector.")
