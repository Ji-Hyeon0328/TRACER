...
def feature_vec(world, beta, theta, anchor_score):
    x = []
    for w in WORLD_KEYS:
...
def add_action_to_world(data, world, action_name, theta, meta):
    ...
    world_obj["actions"].append({
        ...
        "theta_action": theta,
        "source": "theta_residual_actor_v0",
        **meta,
    })
...
def main():
    ...
    ap.add_argument("--teacher-out", default="configs/phase_b_theta_residual_actor_v0/ppo_warmstart_teacher_table_theta_residual_actor_v0.json")
    ap.add_argument("--model", default="artifacts/phase_b_theta_residual_actor_v0/model.pt")
    ...
    model = ResidualActor(input_dim=int(ckpt["input_dim"]))
    model.load_state_dict(ckpt["model_state_dict"])
    model.eval()

    specs = [
        ("earth", "trot_solid_fast", "theta_actor_v0_earth_from_trot_solid_fast"),
        ("stairs_single", "trot_mid", "theta_actor_v0_stairs_from_trot_mid"),
        ("tracer_sponge_firm_flat", "trot_soft_mid_clear", "theta_actor_v0_sponge_from_trot_soft_mid_clear"),
        ("tracer_sponge_firm_flat", "sponge_reach_then_brake", "theta_actor_v0_sponge_from_sponge_reach_then_brake"),
        ("tracer_sponge_firm_flat", "sponge_probe_crawlish", "theta_actor_v0_sponge_from_sponge_probe_crawlish"),
    ]

    print("Generating residual actor actions:")
    for world, anchor, new_name in specs:
        ...
        x = torch.tensor([feature_vec(world, beta, anchor_theta, anchor_score)], dtype=torch.float32)
