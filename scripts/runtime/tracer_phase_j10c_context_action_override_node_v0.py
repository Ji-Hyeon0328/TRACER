...
class J10CContextActionOverride(Node):
    def __init__(self):
        super().__init__("tracer_phase_j10c_context_action_override_node_v0")

        self.input_theta_topic = os.environ.get(
            "TRACER_PHASE_J10C_INPUT_THETA_TOPIC",
            "/tracer/meta_action_theta_shadow",
        )
        self.output_theta_topic = os.environ.get(
            "TRACER_PHASE_J10C_OUTPUT_THETA_TOPIC",
            "/tracer/meta_action_theta_j10c_override_shadow",
        )
        self.context_topic = os.environ.get(
            "TRACER_PHASE_J10C_CONTEXT_TOPIC",
            "/tracer/terrain_context_label",
        )
        self.pub_hz = env_float("TRACER_PHASE_J10C_PUB_HZ", 10.0)
        ...
        # ]
        self.downslope_theta = [
            6.0,
            env_float("TRACER_PHASE_J10C_DOWNSLOPE_VX_SCALE", 0.88),
            env_float("TRACER_PHASE_J10C_DOWNSLOPE_VX_DELTA", 0.0),
            env_float("TRACER_PHASE_J10C_DOWNSLOPE_YAW_GAIN_SCALE", 0.90),
            env_float("TRACER_PHASE_J10C_DOWNSLOPE_BODY_H_DELTA", -0.004),
            env_float("TRACER_PHASE_J10C_DOWNSLOPE_CLEARANCE_DELTA", 0.004),
            env_float("TRACER_PHASE_J10C_DOWNSLOPE_STABILITY_BIAS", 0.30),
            env_float("TRACER_PHASE_J10C_DOWNSLOPE_ENERGY_BIAS", -0.10),
            0.0,
        ]
        ...
        log_dir = Path(os.environ.get(
            "TRACER_PHASE_J10C_LOG_DIR",
            "logs/phase_j/j10c_context_action_override_latest",
        ))
        log_dir.mkdir(parents=True, exist_ok=True)
        self.log_csv = log_dir / "context_action_override_j10c_v0.csv"
        ...
        self.get_logger().info(f"[TRACER:J10C] input_theta={self.input_theta_topic}")
        self.get_logger().info(f"[TRACER:J10C] output_theta={self.output_theta_topic}")
        self.get_logger().info(f"[TRACER:J10C] context_topic={self.context_topic}")
        self.get_logger().info(f"[TRACER:J10C] log_csv={self.log_csv}")
        self.get_logger().info(f"[TRACER:J10C] downslope override theta={self.downslope_theta}")
...
def main():
    rclpy.init()
    node = J10CContextActionOverride()
    try:
        rclpy.spin(node)
