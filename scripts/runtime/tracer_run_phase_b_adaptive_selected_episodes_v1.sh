...
RUN_ROOT="${TRACER_PHASE_B_ADAPTIVE_RUN_ROOT:-$ROOT/artifacts/phase_b_adaptive_selected_episodes_v1_${STAMP}}"
mkdir -p "$RUN_ROOT"

SUMMARY_CSV="$RUN_ROOT/adaptive_summary.csv"

echo "[TRACER] Phase-B adaptive selected episodes v1"
...
echo "episode,selected_profile,guard_reason,reached,min_rel_dist,final_rel_dist,progress,odom_x_delta,max_vx,max_yaw,risk_mismatch,risk_uncertainty,risk_yaw,risk_slip,risk_stability,risk_json,summary_json" \
  > "$SUMMARY_CSV"
...
if [[ -n "${TRACER_PHASE_B_INITIAL_RISK_STATE_JSON:-}" ]]; then
  ...
  python3 "$ROOT/scripts/runtime/tracer_make_phase_b_risk_state_from_summary_v1.py" \
    --ema-alpha "$EMA_ALPHA" \
    --out-json "$RISK_JSON" \
...
for EP in $(seq 1 "$NUM_EPISODES"); do
  ...
  echo
  echo "============================================================"
  echo "[TRACER] adaptive v1 episode $EP/$NUM_EPISODES"
...
echo
echo "============================================================"
echo "[TRACER] adaptive v1 summary"
echo "============================================================"
