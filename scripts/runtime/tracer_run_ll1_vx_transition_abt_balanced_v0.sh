...
END_INDEX="${TRACER_VX_ABC_END_INDEX:-9}"

TS="$(date +%Y%m%d_%H%M%S)"

RUN_ROOT="${TRACER_VX_ABC_RUN_ROOT:-$ROOT/logs/revalidation_vx_transition_abt_${TS}}"
...
TRANSITION_TABLE='flat:0.115,0.320,0.045;start_flat:0.115,0.320,0.045;upslope:0.090,0.320,0.045;rough:0.090,0.320,0.045;downslope:0.090,0.320,0.045;goal_flat:0.090,0.320,0.045;unknown:0.090,0.320,0.045'
...
if condition == "A":
    ...
    flat_rows = [
        row
        for row in valid
        if row.get("context") == "flat"
        and ff(row.get("x")) is not None
        and ff(row.get("vx_cmd")) is not None
    ]
    ...
    late_rows = [
        row
        for row in flat_rows
        if ff(row["x"]) >= transition_end
    ]

    if (
        len(early_rows) < 100
        or len(ramp_rows) < 100
        or len(late_rows) < 20
        or len(difficult_rows) < 500
    ):
        raise SystemExit(
            "[FAIL] insufficient transition validation samples"
        )
    ...
    # The selector updates at approximately 10 Hz while the state/logger
    # stream is approximately 50 Hz. The current x and held VX command
    # are therefore not exactly simultaneous. Validate the ramp using
    # residual and equivalent-position bounds rather than a 5e-5
    ...
    if not formula_residuals:
        raise SystemExit(
            "[FAIL] no finite ramp residuals"
        )
    ...
    print(
        "transition_formula_residual_p95:",
        residual_p95,
    )

    print(
        "transition_formula_residual_max:",
        residual_max,
    )

    print(
        "transition_equivalent_x_error_max_m:",
        equivalent_x_error_max,
    )

    print(
        "transition_ramp_monotonic_increase_count:",
        monotonic_increase_count,
    )

    print(
        "transition_early_high_fraction:",
        early_high_fraction,
    )

    print(
        "transition_late_low_fraction:",
        late_low_fraction,
    )

    print(
        "transition_difficult_low_fraction:",
        difficult_low_fraction,
    )

    print(
        "transition_ramp_vx_min:",
        min(ramp_values),
    )

    print(
        "transition_ramp_vx_max:",
        max(ramp_values),
    )

    if residual_p95 > 1.5e-4:
        raise SystemExit(
            "[FAIL] ramp residual p95 exceeds ZOH-aware bound"
        )

    if residual_max > 3.0e-4:
        raise SystemExit(
            "[FAIL] ramp residual maximum exceeds ZOH-aware bound"
        )

    if equivalent_x_error_max > 0.006:
        raise SystemExit(
            "[FAIL] ramp equivalent x-alignment error exceeds 6 mm"
        )

    if monotonic_increase_count != 0:
        raise SystemExit(
            "[FAIL] transition ramp is not monotonically non-increasing"
        )

    if early_high_fraction < 0.98:
        raise SystemExit(
            "[FAIL] early-flat high-VX coverage below 98%"
        )

    if late_low_fraction < 0.95:
        raise SystemExit(
            "[FAIL] pre-boundary low-VX coverage below 95%"
        )

    if difficult_low_fraction < 0.99:
        raise SystemExit(
            "[FAIL] difficult-terrain low-VX coverage below 99%"
        )

    if not (
        min(ramp_values) <= 0.092
        and max(ramp_values) >= 0.113
    ):
        raise SystemExit(
            "[FAIL] ramp did not span the intended VX range"
        )
...
for item in "${schedule[@]}"; do
  ...
  transition_mode="$(condition_transition_mode "$condition")"
  ...
  (
    ...
    TRACER_PHASE_D4_TRANSITION_MODE="$transition_mode" \
    TRACER_PHASE_D4_TRANSITION_X_START=1.40 \
    TRACER_PHASE_D4_TRANSITION_X_END=1.90 \
    TRACER_PHASE_D4_TRANSITION_VX_HIGH=0.115 \
    TRACER_PHASE_D4_TRANSITION_VX_LOW=0.090 \
    bash \
...
transition_x_start=1.40
transition_x_end=1.90
transition_vx_high=0.115
transition_vx_low=0.090
world=$WORLD
timeout_s=$TIMEOUT_S
...
echo
echo "===== VX TRANSITION A/B/T CAMPAIGN COMPLETE ====="
echo "RUN_ROOT=$RUN_ROOT"
...
echo "[PASS] selected VX transition campaign runs complete"
