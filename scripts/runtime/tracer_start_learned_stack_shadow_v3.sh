...
PORT="${TRACER_SUPERVISED_STACK_PORT:-50430}"
HOST="${TRACER_SUPERVISED_STACK_HOST:-127.0.0.1}"
OUT_DIR="${TRACER_SHADOW_OUT_DIR:-$ROOT/data/shadow_logs_v2}"
TS="$(date +%Y%m%d_%H%M%S)"
OUT_CSV="${TRACER_SHADOW_OUT_CSV:-$OUT_DIR/learned_stack_shadow_v3_${TERRAIN}_${TS}.csv}"
LOG="${TRACER_SHADOW_LOG:-/tmp/tracer_learned_stack_shadow_v3_${TERRAIN}.log}"
...
echo "[TRACER] starting learned stack shadow node v3"
...
pkill -f "tracer_learned_stack_shadow_node_v3.py" || true
sleep 0.3
...
/usr/bin/python3 ros2_ws/src/tracer_a1_qpmc_adapter/scripts/tracer_learned_stack_shadow_node_v3.py \
  > "$LOG" 2>&1 &

PID=$!
echo "[TRACER] shadow v3 node pid=$PID"

sleep 1.0

echo "[TRACER] process check:"
pgrep -af "tracer_learned_stack_shadow_node_v3.py" || true
