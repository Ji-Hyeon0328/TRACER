...
THETA5_MODEL="${TRACER_PHASE_B_THETA5_MODEL:-}"
...
if [[ -z "$THETA5_MODEL" ]]; then
  THETA5_MODEL="$(ls -td artifacts/phase_b_theta5_regressor_v0_* 2>/dev/null | head -1)/phase_b_theta5_regressor_v0.json"
fi

if [[ ! -s "$THETA5_MODEL" ]]; then
  echo "[TRACER][ERROR] theta5 model not found: $THETA5_MODEL"
  exit 1
fi
...
python3 "$ROOT/scripts/runtime/tracer_predict_phase_b_theta5_objective_ram_v0.py" \
  ...
  --out-json "$PRED_JSON" \
  > "$RUN_DIR/theta5_prediction_stdout.json"

echo
echo "========== theta5 prediction =========="
...
print(json.dumps({
    ...
    "profile": {
        "vx_far": p.get("vx_far"),
        "vx_near": p.get("vx_near"),
        "goal_slow_distance": p.get("goal_slow_distance"),
        "body_height": p.get("body_height"),
        "swing_clearance": p.get("swing_clearance"),
        "world_name": p.get("world_name"),
    },
    "raw_prediction": d.get("raw_prediction", {}),
}, indent=2, sort_keys=True))
PY

read -r VX_FAR VX_NEAR SLOW_DIST STOP_DIST GOAL_DIST BODY_HEIGHT SWING_CLEARANCE <<< "$(
python3 - "$PRED_JSON" <<'PY'
import json
import sys
...
echo "[TRACER] predicted slow_distance:   $SLOW_DIST"
echo "[TRACER] predicted body_height:     $BODY_HEIGHT"
echo "[TRACER] predicted swing_clearance: $SWING_CLEARANCE"

TRACER_PHASE_B_WORLD="$WORLD" \
TRACER_PHASE_B_RUN_DIR="$RUN_DIR/episode" \
TRACER_PHASE_B_GOAL_DISTANCE_AHEAD="$GOAL_DIST" \
TRACER_PHASE_B_VX_FAR="$VX_FAR" \
TRACER_PHASE_B_VX_NEAR="$VX_NEAR" \
TRACER_PHASE_B_GOAL_SLOW_DISTANCE="$SLOW_DIST" \
TRACER_PHASE_B_GOAL_STOP_DISTANCE="$STOP_DIST" \
...
"$ROOT/scripts/runtime/tracer_run_phase_b_goal_episode_v0.sh" \
  2>&1 | tee "$RUN_DIR/theta5_episode_runner.log"

EP_SUMMARY="$RUN_DIR/episode/phase_b_summary.json"

if [[ -s "$EP_SUMMARY" ]]; then
  python3 - "$PRED_JSON" "$EP_SUMMARY" "$THETA5_MODEL" <<'PY'
import json
import sys

pred_path, summary_path, model_path = sys.argv[1:]
...
summary["theta5_model"] = model_path
summary["theta5_profile_name"] = p.get("name")
...
print(json.dumps({
    ...
    "theta5_model": model_path,
    "theta5_guard_reasons": summary.get("theta5_guard_reasons"),
    "vx_far": summary.get("vx_far"),
    "vx_near": summary.get("vx_near"),
    "body_height": summary.get("body_height"),
    "swing_clearance": summary.get("swing_clearance"),
