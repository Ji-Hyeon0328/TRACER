...
LOG_DIR="$ROOT/logs/phase_c_mixed_oracle_$(date +%Y%m%d_%H%M%S)"

mkdir -p "$LOG_DIR"
cd "$ROOT"

echo "[TRACER] start Phase-C mixed oracle stack v0"
...
echo
echo "========== 6. start Phase-C oracle schedule node =========="
nohup /usr/bin/python3 \
  scripts/runtime/tracer_phase_c_oracle_schedule_mpc_ref_node_v0.py \
  --goal-x "$GOAL_X" \
  --pub-hz "$PUB_HZ" \
  > "$LOG_DIR/oracle_schedule.log" 2>&1 &
echo $! > "$LOG_DIR/oracle_schedule.pid"
sleep 1.0
...
echo
echo "[TRACER] Phase-C mixed oracle stack is running."
echo "[TRACER] Monitor:"
echo "  tail -f $LOG_DIR/oracle_schedule.log"
