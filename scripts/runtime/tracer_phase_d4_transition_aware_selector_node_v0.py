...
class ContextMetaSelector(Node):
    def __init__(self):
        super().__init__("tracer_phase_d4_transition_aware_selector_node_v0")
        ...
        # Optional transition-aware flat-to-upslope VX scheduling.
        #
        # mode=none:
        #   preserve the original context-table behavior.
        #
        # mode=linear:
        #   use high VX on early flat, linearly decelerate before the
        #   x=2.0 terrain boundary, and reach low VX before upslope entry.
        self.transition_mode = os.environ.get(
            "TRACER_PHASE_D4_TRANSITION_MODE",
            "none",
        ).strip().lower()

        self.transition_x_start = float(
            os.environ.get(
                "TRACER_PHASE_D4_TRANSITION_X_START",
                "1.40",
            )
        )

        self.transition_x_end = float(
            os.environ.get(
                "TRACER_PHASE_D4_TRANSITION_X_END",
                "1.90",
            )
        )

        self.transition_vx_high = float(
            os.environ.get(
                "TRACER_PHASE_D4_TRANSITION_VX_HIGH",
                "0.115",
            )
        )

        self.transition_vx_low = float(
            os.environ.get(
                "TRACER_PHASE_D4_TRANSITION_VX_LOW",
                "0.090",
            )
        )
        ...
        if not (
            self.transition_x_start
            < self.transition_x_end
        ):
            raise ValueError(
                "transition_x_start must be below transition_x_end"
            )
        ...
        self.get_logger().info(
            "transition="
            f"mode={self.transition_mode}, "
            f"x_start={self.transition_x_start:.3f}, "
            f"x_end={self.transition_x_end:.3f}, "
            f"vx_high={self.transition_vx_high:.4f}, "
            f"vx_low={self.transition_vx_low:.4f}"
        )
    ...
    def base_theta(self) -> Theta:
        ...
        return self.action_table.get(
            self.default_label,
            (
                self.transition_vx_low,
                0.320,
                0.045,
            ),
        )
    ...
    def select_theta(self) -> Theta:
        vx, body_h, clearance = self.base_theta()

        self.transition_alpha = 0.0

        if self.transition_mode != "linear":
            return vx, body_h, clearance

        if self.current_context not in {
            "flat",
            "start_flat",
        }:
            return vx, body_h, clearance

        # Fail closed until the first odometry message arrives.
        ...
        if self.x <= self.transition_x_start:
            self.transition_alpha = 0.0
            transition_vx = self.transition_vx_high

        elif self.x >= self.transition_x_end:
            self.transition_alpha = 1.0
            transition_vx = self.transition_vx_low
    ...
    def on_timer(self):
        ...
        if self.seq % int(max(1.0, self.pub_hz)) == 0:
            self.get_logger().info(
                ...
                f"transition_alpha={self.transition_alpha:.4f} "
