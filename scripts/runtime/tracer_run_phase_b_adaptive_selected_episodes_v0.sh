...
NUM_EPISODES="${TRACER_PHASE_B_ADAPTIVE_EPISODES:-4}"
DURATION="${TRACER_PHASE_B_RECORD_DURATION:-30.0}"
SAMPLE_HZ="${TRACER_PHASE_B_SAMPLE_HZ:-20.0}"

STAMP="$(date +%Y%m%d_%H%M%S)"
RUN_ROOT="${TRACER_PHASE_B_ADAPTIVE_RUN_ROOT:-$ROOT/artifacts/phase_b_adaptive_selected_episodes_${STAMP}}"
mkdir -p "$RUN_ROOT"

SUMMARY_CSV="$RUN_ROOT/adaptive_summary.csv"

echo "[TRACER] Phase-B adaptive selected episodes"
...
echo "episode,selected_profile,guard_reason,reached,min_rel_dist,final_rel_dist,progress,odom_x_delta,max_vx,max_yaw,risk_mismatch,risk_uncertainty,risk_slip,risk_stability,risk_json,summary_json" \
  > "$SUMMARY_CSV"
...
if [[ -n "${TRACER_PHASE_B_INITIAL_RISK_STATE_JSON:-}" ]]; then
  ...
  python3 "$ROOT/scripts/runtime/tracer_make_phase_b_risk_state_from_summary_v0.py" \
    --out-json "$RISK_JSON" \
...
for EP in $(seq 1 "$NUM_EPISODES"); do
  ...
  echo
  echo "============================================================"
  echo "[TRACER] adaptive episode $EP/$NUM_EPISODES"
  echo "[TRACER] risk_json: $RISK_JSON"
  echo "[TRACER] ep_dir:    $EP_DIR"
  echo "============================================================"

  TRACER_PHASE_B_SELECTOR_MODEL="$MODEL" \
  ...
  "$ROOT/scripts/runtime/tracer_run_phase_b_selected_goal_episode_v0.sh" \
    2>&1 | tee "$EP_DIR.log"
...
row = {
    "episode": ep,
    "selected_profile": s.get("selected_profile_name"),
    "guard_reason": s.get("selected_profile_guard_reason"),
