...
GOAL_X="${TRACER_PHASE_C_GOAL_X:-8.4}"
...
LOG_DIR="$ROOT/logs/phase_c_mixed_fixed_$(date +%Y%m%d_%H%M%S)"

mkdir -p "$LOG_DIR"
cd "$ROOT"

echo "[TRACER] start Phase-C mixed fixed baseline stack v0"
...
echo
echo "========== 6. start fixed baseline MPC ref node =========="
nohup /usr/bin/python3 \
  scripts/runtime/tracer_phase_c_fixed_mpc_ref_node_v0.py \
  --goal-x "$GOAL_X" \
  --pub-hz "$PUB_HZ" \
  --vx "$VX" \
  --body-height "$BODY_H" \
  --clearance "$CLEARANCE" \
  > "$LOG_DIR/fixed_baseline.log" 2>&1 &
echo $! > "$LOG_DIR/fixed_baseline.pid"
sleep 1.0
...
echo
echo "[TRACER] Phase-C mixed fixed baseline stack is running."
echo "[TRACER] Monitor:"
echo "  tail -f $LOG_DIR/fixed_baseline.log"
