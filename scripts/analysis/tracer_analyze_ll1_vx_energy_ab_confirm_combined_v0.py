...
def markdown(
    ...
    lines = [
        (
            "# TRACER LL1 VX A/B Confirmatory "
            "Combined Analysis"
        ),
        "",
        "## Collection validity",
    ...
    lines.extend([
        ...
        "",
        (
            "- A/B use five balanced paired blocks; C remains "
            "at the original three-run screening count."
        ),
        (
...
def main():
    ...
    if len(manifest_rows) != 13:
        raise SystemExit(
            "expected 13 combined manifest rows, found {}".format(
                len(manifest_rows)
            )
        )
    ...
    write_csv(
        output
        / "run_metrics_vx_energy_confirm_v0.csv",
        runs,
    )

    write_csv(
        output
        / "segment_metrics_vx_energy_confirm_v0.csv",
        segments,
    )

    write_csv(
        output
        / "condition_summary_vx_energy_confirm_v0.csv",
        condition_summary,
    )

    write_csv(
        output
        / "segment_condition_summary_vx_energy_confirm_v0.csv",
        segment_condition_summary,
    )

    write_csv(
        output
        / "paired_effects_vx_energy_confirm_v0.csv",
        paired_effects,
    )
    ...
    (
        output
        / "analysis_vx_energy_confirm_v0.json"
    ).write_text(
        json.dumps(
            payload,
            indent=2,
    ...
    (
        output
        / "summary_vx_energy_confirm_v0.md"
    ).write_text(
        summary + "\n",
        encoding="utf-8",
    )
    ...
    archive = (
        ...
        / (
            root.name
            + "_analysis_vx_energy_confirm_v0.tar.gz"
        )
    )
