...
def load_train_context_groups():
    if not TRAIN_CONTEXT_SOURCE.exists():
        raise FileNotFoundError(
            TRAIN_CONTEXT_SOURCE
        )

    with TRAIN_CONTEXT_SOURCE.open(
        newline="",
    ) as f:
        rows = list(
            csv.DictReader(f)
    ...
    # Use the frozen u30 Balanced rows solely to
    # recover the exact TRAIN context/seed protocol.
    selected = [
        row
        for row in rows
        if (
            int(float(row["update"])) == 30
            and row["beta_name"]
            == "balanced"
        )
    ]
    ...
    if len(selected) != EXPECTED_CONTEXTS:
        raise RuntimeError(
            "Expected exactly 9 frozen TRAIN "
            "contexts from u30 Balanced rows; "
            f"got {len(selected)}"
        )
    ...
    if {
        ...
        raise RuntimeError(
            "Unexpected TRAIN terrain labels: "
            f"{groups}"
        )
...
def main():
    ...
    plan = {
        "schema":
            "icra27_os_t5p3_train_beta_response_atlas_plan_v0",
    ...
    manifest = {
        "schema":
            "icra27_os_t5p3_train_beta_response_atlas_v0",
