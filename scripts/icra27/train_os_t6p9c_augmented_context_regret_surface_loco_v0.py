...
OUT_FOLDS = (
    OUT_DIR
    / "regret_surface_fold_results.csv"
)
...
OUT_SUMMARY = (
    OUT_DIR
    / "regret_surface_summary.csv"
)

OUT_MANIFEST = (
    OUT_DIR
    / "regret_surface_manifest.json"
)
...
def main():
    ...
    for representation in REPRESENTATIONS:
        ...
        for fold_index, held in enumerate(
            ...
            # Network output was trained on log1p(regret).
            pred_log_regret = pred_J
            ...
            # Training target is log1p(physical regret).
            train_log_regret_mae = np.mean(
                np.abs(
                    pred_train_J
                    - Y_train
                ),
            ...
            # Decision-relevant held metric is measured
            # after mapping the network output back to
            # physical regret space.
            held_regret_mae = np.mean(
                np.abs(
                    pred_regret
                    - true_regret
                ),
            ...
            fold_row = {
                ...
                "train_mae_log_regret_motion":
                    float(
                        train_log_regret_mae[
                            0
                        ]
                    ),

                "train_mae_log_regret_stability":
                    float(
                        train_log_regret_mae[
                            1
                        ]
    ...
    for representation in REPRESENTATIONS:
        ...
        summary = {
            ...
            "held_mae_regret_stability_mean":
                float(
                    np.mean(
                        values(
                            "held_mae_regret_stability"
    ...
    manifest = {
        "schema":
            "icra27_os_t6p9c_augmented_context_regret_surface_loco_v0",
        ...
        "objective":
            (
                ...
                "normalized physical-regret surrogate "
                "g(context,beta)->"
                "[r_motion,r_stability,r_energy]."
            ),
        ...
        "important_semantics":
            (
                "The regret surrogate is eta-agnostic. "
                "Once r(context,beta) is predicted, "
