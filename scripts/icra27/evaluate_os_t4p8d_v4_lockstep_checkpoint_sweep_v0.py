...
def main():
    ...
    # --------------------------------------------------------
    # OS-T4.8d helper reward-log schema promotion.
    #
    # evaluate_os_t4p3c_train9_cost_basis_v0 is intentionally
    # retained as the historical v3 metric reader/summarizer.
    # Its cost-vector algebra is still valid for v4 because
    # v3 -> v4 changes only traction temporal provenance.
    #
    # Promote ONLY the dynamically loaded helper instance.
    # Do not modify the shared helper source.
    # --------------------------------------------------------

    historical_helper_schema = (
        helper.EXPECTED_REWARD_SCHEMA
    )

    if historical_helper_schema != (
        "icra27_simplified_tracer_cost_v3"
    ):
        raise RuntimeError(
            "Unexpected historical helper reward schema: "
            f"{historical_helper_schema!r}"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "OS-T4.8d helper schema promotion failed."
        )
    ...
    if not hasattr(
        ...
        raise RuntimeError(
            "phase2b module does not expose "
            "PyMPCM7Env; refusing lockstep override."
        )

    original_pympc_env_ctor = (
        phase2b.PyMPCM7Env
    )

    def lockstep_pympc_env_ctor(
        *args,
        **kwargs,
    ):
        if (
            "lockstep_eval" in kwargs
            and not bool(
                kwargs["lockstep_eval"]
            )
        ):
            raise RuntimeError(
                "OS-T4.8d attempted to "
                "disable lockstep evaluation."
            )
        ...
        if (
            ...
            raise RuntimeError(
                "OS-T4.8d requires exactly "
                "100 physics steps per HL step."
            )
        ...
        # OS-T4.8d:
        # evaluator-local final authority.
        #
        # Do not rely only on imported module globals:
        # force the actual PyMPCM7Env constructor request
        # to use the v4 latched-traction reward contract.
        kwargs[
            "reward_mode"
        ] = "tracer_cost_v4"
        ...
        if not bool(
            ...
            raise RuntimeError(
                "Constructed environment is "
                "not in lockstep mode."
            )
        ...
        if int(
            ...
            raise RuntimeError(
                "Constructed environment has "
                "wrong lockstep step count."
            )
        ...
        if (
            ...
            raise RuntimeError(
                "OS-T4.8d constructed environment "
                "has wrong reward mode: "
                f"{env.reward_mode!r}"
            )
    ...
    phase2b.PyMPCM7Env = (
        lockstep_pympc_env_ctor
    )

    # --------------------------------------------------------
    # OS-T4.8d reward-mode promotion.
    #
    # There are TWO independently loaded evaluator globals:
    #
    #   phase2b.base.EVAL_REWARD_MODE
    #       common evaluator / reward_components contract
    #
    #   phase2b.EVAL_REWARD_MODE
    #       phase2b.make_env() reward mode
    #
    # Both must be promoted atomically.
    # --------------------------------------------------------
    ...
    # Both independently loaded evaluator modules retain
    # their historical tracer_cost_v2 defaults.
    #
    # OS-T4.8d promotes both atomically to tracer_cost_v4
    # without modifying the shared evaluator source.
    ...
    if (
        ...
        raise RuntimeError(
            "OS-T4.8d reward-mode promotion failed."
        )
    ...
    print("=" * 112)
    print(
        "ICRA27 OS-T4.8d LOCKSTEP TRAIN-ONLY "
        "BETA CHECKPOINT SEMANTIC SWEEP"
    )
    print("=" * 112)
    ...
    print(
        "terminal   : early native/M4 boundary enabled"
    )
    print(
    ...
    if selected is None:
        ...
        print(
            "[ICRA27] OS-T4.8d v4 checkpoint semantic gate: PASS"
        )
        print(
            "selected candidate:",
    ...
    manifest = {
        "schema":
            "icra27_os_t4p8d_v4_lockstep_checkpoint_sweep_v0",
