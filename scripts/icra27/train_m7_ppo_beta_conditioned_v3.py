...
def phase1a_v2_std(
    *,
    ...
    """
    Deterministic linear exploration schedule.

    update_index is zero-based.

    For one-update smoke runs, retain start_std.
    """
    ...
    if total_updates <= 0:
        raise ValueError(
            "total_updates must be positive"
        )
...
def set_fixed_policy_std(
    model,
    std,
):
    """
    Set the global Gaussian std explicitly.

    log_std remains frozen; PPO is not allowed to learn it.
    """
    std = float(std)

    if not np.isfinite(std) or std <= 0.0:
        raise ValueError(
            f"Invalid exploration std: {std}"
        )
...
def normalize_condition_advantages(
    batch,
    *,
    steps_per_condition,
    condition_count,
    ...
    """
    Normalize actor advantages independently for each
    contiguous (terrain, beta) rollout segment.

    IMPORTANT:
      - returns are NOT changed;
      - critic targets therefore retain original reward scale;
      - only actor advantages are condition-balanced.

    ppo_update() performs its historical global normalization
    afterward. Because all equally-sized condition slices are
    already approximately N(0,1), that second normalization is
    effectively a no-op apart from floating-point error.
    """
    condition_size = int(
        steps_per_condition
    )
    ...
    if advantages.shape != (
        ...
        raise RuntimeError(
            "Condition advantage shape mismatch: "
            f"expected=({expected},), "
...
def main():
    ...
    ap.add_argument(
        ...
        default=0.05,
        help=(
            "Final fixed Gaussian exploration std. "
            "Linearly annealed from --initial-std "
            "across PPO updates."
        ),
    )
    ...
    ap.add_argument(
        ...
        default=(
            "results/icra27/"
            "m7_ppo_beta_conditioned_tracer_cost_v3_lr1p5e4_v0"
        ),
    )
    ...
    if args.initial_std <= 0.0:
        raise ValueError(
            "--initial-std must be > 0"
        )

    if args.final_std <= 0.0:
        raise ValueError(
            "--final-std must be > 0"
        )

    if args.final_std > args.initial_std:
        raise ValueError(
            "--final-std must be <= --initial-std "
            "for the Phase-1A-v2 annealing contract."
        )
    ...
    cfg = PPOConfig(
        ...
        # Phase-1A-v2:
        # exploration is controlled explicitly by the
        # deterministic std schedule below.
        entropy_coef=0.0,
    ...
    # Phase-1A-v2:
    # do not let PPO/entropy optimization change exploration
    # variance. We control it explicitly once per update.
    model.log_std.requires_grad_(
        False
    )
    ...
    summary = {
        "schema":
            "icra27_m7_ppo_beta_conditioned_v3",
        ...
        "phase1a_v2_training":
            {
                ...
                "advantage_normalization":
                    (
                        "per_condition_before_"
                        "historical_global_normalization"
                    ),
    ...
    print("=" * 88)
    print(
        "ICRA27 M7 BETA-CONDITIONED MULTI-TERRAIN PPO / TRACER-COST-V3 / LR-1.5E-4"
    )
    print("=" * 88)
    ...
    print(
        ...
        f"-> {args.final_std:.3f} "
        "(linear, fixed per update)"
    )

    print(
        "entropy coef       : "
        f"{cfg.entropy_coef:.3f}"
    )

    print(
        "adv normalization  : "
        "per (terrain,beta) condition"
    )
    ...
    for update_i in range(
        ...
        current_std = phase1a_v2_std(
            update_index=update_i,
            total_updates=args.updates,
            start_std=args.initial_std,
            final_std=args.final_std,
        )

        set_fixed_policy_std(
            model,
            current_std,
        )
        ...
        ) = normalize_condition_advantages(
            batch,
            steps_per_condition=(
                args.steps_per_condition
            ),
    ...
    print(
        ...
        "PPO tracer-cost-v3 LR-half training: PASS"
    )
    print("=" * 88)
