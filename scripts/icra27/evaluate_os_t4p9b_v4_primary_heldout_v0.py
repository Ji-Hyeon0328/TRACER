...
EVAL_REWARD_MODE = "tracer_cost_v4"
...
def make_env(
    *,
    ...
    base_env = PyMPCM7Env(
        ...
        # OS-T4.9b:
        # exact transition contract inherited from
        # train-only T4.8d checkpoint selection.
        lockstep_eval=True,
        lockstep_physics_steps=100,
    ...
    # --------------------------------------------------------
    # OS-T4.9b constructed base-environment contract.
    # --------------------------------------------------------

    if (
        str(base_env.reward_mode)
        != "tracer_cost_v4"
    ):
        raise RuntimeError(
            "OS-T4.9b constructed base env "
            "has wrong reward mode: "
            f"{base_env.reward_mode!r}"
        )

    if not bool(
        base_env.lockstep_eval
    ):
        raise RuntimeError(
            "OS-T4.9b constructed base env "
            "is not lockstep."
    ...
    if int(
        base_env.lockstep_physics_steps
    ) != 100:
        raise RuntimeError(
            "OS-T4.9b requires exactly "
            "100 physics ticks per HL decision."
        )
...
def main():
    ...
    # --------------------------------------------------------
    # OS-T4.9b common evaluator contract.
    # --------------------------------------------------------

    historical_base_reward_mode = (
        base.EVAL_REWARD_MODE
    )

    if historical_base_reward_mode != (
        "tracer_cost_v2"
    ):
        raise RuntimeError(
            "Unexpected historical common-evaluator "
            "reward mode: "
            f"{historical_base_reward_mode!r}"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "OS-T4.9b requires "
            "tracer_cost_v4 checkpoint provenance."
        )
    ...
    summary = {
        "schema":
            "icra27_os_t4p9b_v4_primary_heldout_v0",
