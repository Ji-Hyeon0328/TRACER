...
def main():
    ...
    if len(rows) % beta_count != 0:
        raise RuntimeError(
            "Candidate row count is not divisible "
            "by beta-bank size: "
            f"rows={len(rows)}, betas={beta_count}"
        )
    ...
    if len(records) != expected_record_count:
        raise RuntimeError(
            "Exact-support record-count mismatch: "
    ...
    summary = {
        "schema":
            "icra27_phase2b_inverse_beta_exact_v0",
    ...
    print(
        "ICRA27 PHASE-2B EXACT INVERSE-BETA SUPPORT V0"
    )
    ...
    print()
    print(
        "[ICRA27] Phase-2B exact inverse-beta "
        "support: PASS"
    )
