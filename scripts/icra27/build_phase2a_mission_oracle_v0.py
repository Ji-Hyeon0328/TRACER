...
def main():
    ...
    if len(sa_rows) != 116:
        raise RuntimeError(
            f"Expected 116 self-assessment rows, "
            f"got {len(sa_rows)}"
        )

    if len(inv_rows) != 116:
        raise RuntimeError(
            f"Expected 116 inverse rows, "
            f"got {len(inv_rows)}"
        )
    ...
    if len(contexts) != 29:
        raise RuntimeError(
            f"Expected 29 good-trajectory contexts, "
            f"got {len(contexts)}"
        )
    ...
    summary = {
        "schema":
            "icra27_phase2a_mission_oracle_v0",
    ...
    print(
        "ICRA27 PHASE-2A COUNTERFACTUAL "
        "MISSION ORACLE V0"
    )
