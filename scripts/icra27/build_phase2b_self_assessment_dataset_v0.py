...
def main():
    ...
    if len(rows) % len(EXPECTED_BETAS) != 0:
        raise RuntimeError(
            "Candidate row count is not divisible by "
            f"beta-bank size: rows={len(rows)}, "
            f"betas={len(EXPECTED_BETAS)}"
        )
    ...
    summary = {
        "schema":
            "icra27_phase2b_self_assessment_v0",
    ...
    print(
        "ICRA27 PHASE-2B SELF-ASSESSMENT DATASET V0"
    )
    ...
    print()
    print(
        "[ICRA27] Phase-2B self-assessment "
        "dataset: PASS"
    )
