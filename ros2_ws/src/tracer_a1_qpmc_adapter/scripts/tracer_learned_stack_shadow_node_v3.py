...
def _code_norm(label: Any, table: dict, denom: float, default: str = "unknown") -> float:
    key = normalize_label(label) if table is RAM_SHADOW_GMS_CODE else str(label or default).strip()
    return f(table.get(key, table.get(default, 0))) / max(1.0, denom)




def objective_v2_beta_prior_for_shadow(terrain: str) -> List[float]:
    # Approximate objective_selector_v2 outputs observed in dataset/server smoke.
    # Used only to keep RAM-shadow-v2 input window in-distribution before UDP response.
...
def build_ram_shadow_step(dbg: Dict[str, Any], step: Dict[str, float], rule_label: str) -> Dict[str, float]:
    # This builds the RAM shadow v2 feature vector used by artifacts/ram_shadow_v2.
    # For the current step, learned outputs are not known until after the UDP query.
    # We therefore initialize them from rule/proxy signals and overwrite them after response.
    terrain = str(dbg.get("terrain", "unknown"))
    beta = objective_v2_beta_prior_for_shadow(terrain)

    proxy_intervention = max(
        f(step.get("gate_would_override", 0.0)),
        f(step.get("gate_control_risk", 0.0)),
        f(step.get("gate_fallen_prob", 0.0)),
        f(step.get("gate_recovery_prob", 0.0)),
    )

    return {
        "final_vx": f(step.get("mpc_vx", 0.0)),
        "final_yaw": f(step.get("mpc_yaw", 0.0)),
        "final_body_height": f(step.get("mpc_body_height", 0.0)),
        "final_clearance": f(step.get("mpc_clearance", 0.0)),
        "final_enable": f(step.get("mpc_enable", 1.0)),

        "rule_gms_code": _code_norm(rule_label, RAM_SHADOW_GMS_CODE, 5.0),
        "learned_gms_code": _code_norm(rule_label, RAM_SHADOW_GMS_CODE, 5.0),
        ...
        "learned_ram_future_override_mean": f(step.get("gate_would_override", 0.0)),

        "rule_ram_level_code": f(step.get("gate_level_code", 0.0)) / 3.0,
        "rule_gate_action_code": f(step.get("gate_action_code", 0.0)) / 4.0,
        "rule_control_risk": f(step.get("gate_control_risk", 0.0)),
        "rule_fallen_prob_weak": f(step.get("gate_fallen_prob", 0.0)),
        "rule_sigma_mean": f(step.get("gate_sigma_mean", 0.0)),
    }


def update_ram_shadow_step_from_response(ram_step: Dict[str, float], resp: Dict[str, Any], rule_label: str):
    obj = resp.get("objective") or {}
    ram = resp.get("ram") or {}
    gms = resp.get("gms") or {}

    beta = obj.get("beta") or []
    if len(beta) >= 3:
        ram_step["learned_beta_motion"] = f(beta[0])
        ram_step["learned_beta_stability"] = f(beta[1])
        ram_step["learned_beta_energy"] = f(beta[2])

    # Important:
    # Do NOT feed RAM shadow v3 predictions back into its own future input window.
    # The RAM input window should remain a teacher/proxy observation stream.
    # Otherwise an early high RAM prediction can create a self-amplifying feedback loop.
    learned_label = normalize_label(gms.get("label", rule_label))
    ram_step["learned_gms_code"] = _code_norm(learned_label, RAM_SHADOW_GMS_CODE, 5.0)
    ram_step["learned_gms_prob"] = f(gms.get("prob", ram_step.get("learned_gms_prob", 1.0)))
    ram_step["gms_disagree"] = 1.0 if learned_label != normalize_label(rule_label) else 0.0
...
class LearnedStackShadowNodeV3(Node):
    def __init__(self):
        super().__init__("tracer_learned_stack_shadow_node_v3")

        self.host = os.environ.get("TRACER_SUPERVISED_STACK_HOST", "127.0.0.1")
        self.port = env_int("TRACER_SUPERVISED_STACK_PORT", 50430)
        ...
        default_out = f"data/shadow_logs_v2/learned_stack_shadow_v3_{terrain}_{int(time.time())}.csv"
        ...
        self.get_logger().info(
            f"learned stack shadow v3 started udp={self.host}:{self.port} "
            f"out={self.out_csv} duration={self.duration_sec}"
        )

    def on_timer(self):
        if self.duration_sec > 0.0 and (time.time() - self.t0) >= self.duration_sec:
            self.get_logger().info(f"shadow v3 duration reached; closing csv={self.out_csv}")
            self.fobj.flush()
            self.fobj.close()
            os._exit(0)
    ...
    def on_debug(self, msg: String):
        ...
        ram_step = build_ram_shadow_step(dbg, step, rule_label)
        self.window.append(ram_step)

        payload = {
            "request_id": f"shadow_v3_{terrain}_{self.n}",
            "terrain": terrain,
        ...
        try:
            ...
            update_ram_shadow_step_from_response(ram_step, resp, rule_label)
        ...
        if self.n % 25 == 0:
            self.get_logger().info(
                f"shadow_v3 n={self.n} terrain={terrain} "
