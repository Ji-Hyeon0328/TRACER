...
class TracerLearnedHighLevelPolicyUdpClientNode(Node):
    """
    ROS2 wrapper for learned high-level policy.
    ...
    UDP request:
      36 doubles = relative_goal[4] + context[16] + latent[16]

    UDP response:
      5 doubles = [vx, yaw_rate, body_height, clearance, enable]
    ...
    Torch inference runs in env_isaaclab through UDP server.
    """

    def __init__(self):
        super().__init__("tracer_learned_high_level_policy_udp_client_node")
        ...
        self.declare_parameter("server_port", 50210)
        ...
        self.get_logger().info(
            f"learned HL policy UDP client: "
    ...
    def timer_callback(self):
        ...
        request = self.last_goal + self.last_context + self.last_latent
    ...
    def poll_response(self):
        while True:
            ...
            if len(data) < self.output_size:
                self.get_logger().warn(f"short learned HL policy response: {len(data)} bytes")
                continue
            ...
            if now - self.last_log_wall > 1.0:
                self.get_logger().info(
                    f"learned HL response from {addr[0]}:{addr[1]} "
...
def main():
    rclpy.init()
    node = TracerLearnedHighLevelPolicyUdpClientNode()
    try:
        rclpy.spin(node)
    finally:
        node.destroy_node()
