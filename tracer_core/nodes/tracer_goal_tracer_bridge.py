...
class GoalTracerBridge(object):
    ...
    def __init__(self):
        ...
        # M14: yaw modulation by TRACER mode.
        # Yaw is still generated by GoalTracker, but TRACER can attenuate it
        # under mismatch/posture-conservative modes.
        self.yaw_scale_nominal = rospy.get_param("~yaw_scale_nominal", 1.00)
        self.yaw_scale_cautious = rospy.get_param("~yaw_scale_cautious", 0.85)
        self.yaw_scale_conservative = rospy.get_param("~yaw_scale_conservative", 0.65)
        self.yaw_scale_posture = rospy.get_param("~yaw_scale_posture", 0.50)
    ...
    def yaw_scale_from_mode(self, mode):
        if mode == "nominal":
            return self.yaw_scale_nominal
        if mode == "cautious_mismatch":
            return self.yaw_scale_cautious
        if mode == "conservative_mismatch":
            return self.yaw_scale_conservative
        if mode == "conservative_posture":
            return self.yaw_scale_posture
        if mode == "recovery":
            return 0.0
        return 1.0
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # M14: mode-conditioned yaw modulation.
            yaw_scale = self.yaw_scale_from_mode(tracer["mode"])
            yaw_axis = raw_yaw_axis * yaw_scale
            yaw_axis = clamp(yaw_axis, -self.max_yaw_axis, self.max_yaw_axis)
