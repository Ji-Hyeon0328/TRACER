...
class HighLevelPolicyMLP(nn.Module):
    def __init__(self, input_dim=36, output_dim=5, hidden_dim=128):
        super().__init__()
...
def main():
    ...
    parser.add_argument(
        "--checkpoint",
        default="/home/shoko/Tracer/TRACER/checkpoints/tracer_high_level_policy_v0_best.pt",
    )
    parser.add_argument("--udp-ip", default="0.0.0.0")
    parser.add_argument("--udp-port", type=int, default=50210)
    parser.add_argument("--cpu", action="store_true")
    parser.add_argument("--threshold-enable", action="store_true", default=True)
    ...
    print(f"[TRACER] learned high-level policy UDP server listening on {args.udp_ip}:{args.udp_port}")
    print("[TRACER] input:  36 doubles = relative_goal[4] + context[16] + latent[16]")
    print("[TRACER] output: 5 doubles = [vx, yaw_rate, body_height, clearance, enable]")
    ...
    while True:
        ...
        x = np.asarray(values, dtype=np.float32).reshape(1, 36)
        ...
        # Safety clamp before sending command.
        ...
        # For low-level safety, use binary enable.
