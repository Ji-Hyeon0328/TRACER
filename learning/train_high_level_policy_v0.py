...
def load_dataset(data_dir: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for path in files:
        ...
        # Input: relative_goal[4] + context[16] + latent[16]
        x = np.concatenate(
            [
                relative_goal[valid],
                context[valid],
        ...
        # Target: [vx, yaw_rate, body_height, clearance, enable]
        # mpc layout = [counter, vx, yaw_rate, body_height, clearance, enable]
        y = mpc[valid][:, 1:6]
...
def train(args):
    ...
    best_val = float("inf")
    best_path = os.path.join(args.out_dir, "tracer_high_level_policy_v0_best.pt")
    final_path = os.path.join(args.out_dir, "tracer_high_level_policy_v0_final.pt")
    ts_path = os.path.join(args.out_dir, "tracer_high_level_policy_v0.ts")
